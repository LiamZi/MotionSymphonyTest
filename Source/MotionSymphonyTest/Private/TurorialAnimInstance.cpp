...
void UTurorialAnimInstance::SetupMotionMatching()
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Warning: MM Animation Blueprint - Could not set the input profile on the trajectory generator as it is not valid. Are you sure the character has a valid Trajectory Generator Component added?"));
	}
}
...
void UTurorialAnimInstance::TickAnimation()
{
	...
	{
		//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Error: The Animation must need a character in this tick."));
...
void UTurorialAnimInstance::StrafingCheck()
{
	...
	_isStrafing = (_trajectoryGenerator->TrajectoryBehaviour == ETrajectoryMoveMode::Strafe);
}

void UTurorialAnimInstance::ConstructInputResponseData()
{
	//if (_mmInputResponseData.DesiredInputArray.Num() <= 0 || !_character || !_mmAnimConfig)
	if(!_character || !_mmAnimConfig)
	{
		GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, TEXT("Error: The Animation must need a Input Response Data, Character And a AnimConfig."));
		return;
	}

	UMMBlueprintFunctionLibrary::ConstructMotionInputFeatureArray(_mmInputResponseData, _character, _mmAnimConfig);
}
